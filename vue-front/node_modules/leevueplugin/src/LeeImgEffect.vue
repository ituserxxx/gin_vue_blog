<template>
    <div class="lee-img-effect" :class="theeffect" :style="'height:'+height">
        <div class="control" v-if="theeffect=='change'||theeffect=='slideleft'">
            <div class="arrow arrowleft" @click="prev"></div>
            <div class="arrow arrowright" @click="next"></div>
        </div>
        <div v-if="theeffect=='change'" class="fade">
            <div class="text" :class="whichclasst">{{bg[cur].tit}}</div>
            <img :src="bg[cur].imgurl" alt="" :class="whichclass">
        </div>
        <ul v-else-if="theeffect=='ninegrid'">
            <li v-for="item in config">
                <h2>{{item.tit}}</h2>
                <img :src="item.imgurl">
            </li>
        </ul>
        <ul v-else-if="theeffect=='slideleft'" :style="'width:'+totalw+'px;transform:translateX(-'+(cur*sw)+'px)'">
...
export default {
    ...
    mounted() {
        var _this = this
        if (this.theeffect === 'change') {
            var img = document.querySelector('.fade').children
            this.setint = setInterval(this.addf, 5000)
        } else if (this.theeffect === 'slideleft') {
    ...
    watch: {
        ...
        effect: {
            immediate: true,
            handler(value) {
                let types=['accordion','change','slideleft','ninegrid']
