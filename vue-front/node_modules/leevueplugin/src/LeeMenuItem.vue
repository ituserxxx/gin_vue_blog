<template>
    <li class="lee-menu-item" :class="{active:exp}"><span @click="toggle(item)"><i v-if="item.child&&!exp">+</i><i v-else>-</i>{{item.text}}</span>
        <transition name="leetran" @enter="enter" @after-enter="afterenter" @leave="leave" @after-leave="afterleave">
            <ul class="lee-menu-sub" v-show="exp" v-if="item.child">
                <lee-menu-item v-for="(items, index) in item.child" :item="items" :key="index"></lee-menu-item>
            </ul>
        </transition>
    </li>
</template>
